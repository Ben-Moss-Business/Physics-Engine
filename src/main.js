...
let gravity_mult = document.getElementById("grav").value;
let elastic = document.getElementById("elas").value;
let friction = 0.05 * gravity_mult;
...
class Ball {
  ...
  display() {
    ctx.font = "10px sans-serif"; // Reset font size for ball text
...
function keyControl(b) {
  canvas.addEventListener("keydown", function (e) {
    if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.key)) {
      e.preventDefault(); // stop default scrolling or input movement
    }
    if (e.keyCode === 37) {
      LEFT = true;
    }
...
function total_kinetic_energy_display() {
  let totalKE = 0;
  BALLZ.forEach((b) => {
    let speed = b.vel.mag();
    totalKE += 0.5 * b.m * speed * speed;
  });
  ...
  ctx.textAlign = "left";
  ctx.fillText(
    "Kinetic Energy: " + round(totalKE, 2) + " J",
    10,
    canvas.height - 10
  );
}
function mainLoop(timestamp) {
  gravity_mult = document.getElementById("grav").value;
  elastic = document.getElementById("elas").value;
  friction = 0.05 * gravity_mult;

  ctx.clearRect(0, 0, canvas.clientWidth, canvas.clientHeight);
  BALLZ.forEach((b, index) => {
    let rawElastic = parseFloat(elastic);
    if (isNaN(rawElastic) || rawElastic < 0 || rawElastic > 1) {
      rawElastic = 1; // fallback to default safe value
    }
    b.elasticity = rawElastic;
    b.drawBall();
    if (b.player) {
      keyControl(b);
    }
...
for (let i = 0; i < 10; i++) {
  ...
  newBall.elasticity = 1 * elastic; //randInt(0, 10) / 10;
}

let Wall1 = new Wall(500, 100, 750, 200);
